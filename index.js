...
function changeMultiplayer(val) {
    ...
    if (playState.multiplayer === 2) {
        ...
        $id("options").innerHTML = `<button class="start-btn" onclick="confirmStart()">INICIAR VS IA</button>`;
    }
}

function confirmStart() {
    const name1 = $id("n1")?.value.trim() || "Jugador 1";
    const name2 = $id("n2")?.value.trim() || (playState.multiplayer === 1 ? "Bot Gemini" : "Jugador 2");
...
function startGame() {
    ...
    $id("options").innerHTML = `<p style="font-size:0.7rem; opacity:0.5">RONDA ${4 - playState.numPlays} DE 3</p>`;
...
function checkWin(y, x) {
    ...
    const isW = (b[y].every(c=>c===p) || b.every(r=>r[x]===p) || 
                (y===x && b.every((r,i)=>r[i]===p)) || (y+x===2 && b.every((r,i)=>r[2-i]===p)));
...
function handleEnd(type) {
    ...
    $id("announcement-text").innerText = p1Status === "ganas" ? "RONDA GANADA" : (p1Status === "empate" ? "EMPATE" : "RONDA PERDIDA");
...
function renderSummary() {
    ...
    const getBadges = (p) => Object.values(p.state).map(s => {
        const char = s.status === 'ganas' ? 'W' : s.status === 'empate' ? 'D' : 'L';
        return `<div class="badge ${s.status}">${char}</div>`;
    }).join("");

    final.innerHTML = `
        <h2 style="letter-spacing:4px; margin-bottom:40px; font-weight:300">RESUMEN</h2>
        <div style="display:flex; justify-content:space-between; margin-bottom:40px">
            <div style="flex:1">
                <p style="opacity:0.5; font-size:0.7rem">${players[1].name}</p>
                <h3 style="font-size:2.5rem">${players[1].wins}</h3>
                <div class="badge-container">${getBadges(players[1])}</div>
            </div>
            <div style="flex:1">
                <p style="opacity:0.5; font-size:0.7rem">${players[2].name}</p>
                <h3 style="font-size:2.5rem">${players[2].wins}</h3>
                <div class="badge-container">${getBadges(players[2])}</div>
            </div>
        </div>
        <button class="reset-btn" onclick="location.reload()">NUEVA SERIE</button>
    `;
}
